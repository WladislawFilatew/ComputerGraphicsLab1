    ...
    internal class Grad:Filters
    {
        ...
        public override Bitmap processImage(Bitmap image, System.ComponentModel.BackgroundWorker backgroundWorker1)
        {
            ...
            for (int y = 0 ; y < image.Height; y++)
            {
                ...
                {
                    Color dilatedColor = dilation.GetPixel(x, y);
                    Color erodedColor = erosion.GetPixel(x, y);

                    int diffR = Clamp(dilatedColor.R - erodedColor.R,0,255);
                    int diffG = Clamp(dilatedColor.G - erodedColor.G,0,255);
                    int diffB = Clamp(dilatedColor.B - erodedColor.B,0,255);
